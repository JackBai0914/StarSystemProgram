...
public:
    DependencyCalculator(int vertex) :
            vertex_(vertex) {
        init_();
...
private:
    ...
    void init_() {
        shortest_paths_.resize(graph.get_number_vertices());
        dependency_.resize(graph.get_number_vertices());

        shortest_path_predecessors_.resize(graph.get_number_vertices());
        distance_.resize(graph.get_number_vertices());
...
void update_betweenness(int vertex, DependencyCalculator& dc) {
    ...
    int mx = graph.get_number_vertices();
    for (int v = 0; v < mx; v++) {
        if (v != vertex) {
            cerr << "adding " << dc.dependency_[v] << endl;
            betweenness[v] += dc.dependency_[v];
        }
    }
    // ti += TIME;
}
...
int main(int argc, char *argv[]) {
    ...
    parse_input();
    cerr << "finish input " << TIME << endl;
    init();
    cerr << "finish init " << TIME << endl;
    launch_threads();
    join_threads();
    cerr << "end threads " << TIME << endl;

    print_betweenness();
    cerr << "after print " << TIME << endl;


    cerr << "time in update: " << ti << endl;
    return 0;
}